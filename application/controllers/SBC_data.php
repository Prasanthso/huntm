...
class SBC_data extends CI_Controller {
    ...
    public function get_customers_for_messaging() {
        try {
            ...
            $total_customers = $this->CustomerRegister_model->get_filtered_customers_count($status, $scheme, $area);
    ...
    public function send_batch_messages() {
        ...
        try {
            ...
            log_message('debug', "Large batch request: batch_size=$batch_size, current_batch=$current_batch, total_customers=$total_customers");

            // Get customers in chunks for memory efficiency
            $offset = $current_batch * $batch_size;
            $batch_customers = $this->CustomerRegister_model->get_filtered_customers_chunk(

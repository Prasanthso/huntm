...
//      public function get_nil_refill_data() {
//         $userid = $this->session->userdata('id');
//         if (!$userid) {
//             log_message('error', 'User ID not found in session for nil refill data');
//             return false;
//         }
...
//             log_message('info', 'No nil refill data found for user ID: ' . $userid);
//             return [];
//         }
...
//             // Log unexpected scheme values for debugging
//             if (!in_array($row['scheme_selected'], ['PMUY', 'Non PMUY', 'Ujjwala', 'Ujjwala - Extended'])) {
//                 log_message('warning', 'Unexpected scheme value: ' . $row['scheme_selected']);
//             }
...
//                 } catch (Exception $e) {
//                     log_message('error', 'Invalid last_refill_date for consumer_id: ' . $row['consumer_id']);
...
//     public function get_nil_refill_stats($customers) {
//         $counts = [
...
//                 $days = (int)$row['days_since_refill'];
//                 $status = strtolower($row['consumer_sub_status']);

//                 // Count for > 3 months
//                 if ($days > 90) {
//                     if (isset($counts['greater_than_3_months'][$status])) {
//                         $counts['greater_than_3_months'][$status][$is_pmuy ? 'pmuy' : 'non_pmuy']++;
//                         $counts['greater_than_3_months'][$status]['total']++;
//                         $counts['greater_than_3_months']['total'][$is_pmuy ? 'pmuy' : 'non_pmuy']++;
//                         $counts['greater_than_3_months']['total']['total']++;
//                     }
//                 }
//                 // Count for > 6 months
//                 if ($days > 180) {
//                     if (isset($counts['greater_than_6_months'][$status])) {
//                         $counts['greater_than_6_months'][$status][$is_pmuy ? 'pmuy' : 'non_pmuy']++;
//                         $counts['greater_than_6_months'][$status]['total']++;
//                         $counts['greater_than_6_months']['total'][$is_pmuy ? 'pmuy' : 'non_pmuy']++;
//                         $counts['greater_than_6_months']['total']['total']++;
//                     }
//                 }
//                 // Count for > 1 year
//                 if ($days > 365) {
//                     if (isset($counts['greater_than_1_year'][$status])) {
//                         $counts['greater_than_1_year'][$status][$is_pmuy ? 'pmuy' : 'non_pmuy']++;
//                         $counts['greater_than_1_year']['total']['total']++;
//                         $counts['greater_than_1_year']['total'][$is_pmuy ? 'pmuy' : 'non_pmuy']++;
//                         $counts['greater_than_1_year']['total']['total']++;
//                     }
//                 }
//                 // Count for total (all nil-refill customers)
//                 if ($days > 90) {
//                     $counts['total']['total'][$is_pmuy ? 'pmuy' : 'non_pmuy']++;
//                     $counts['total']['total']['total']++;
//                 }
//             }
//         }

//         // Calculate percentages using total nil-refill customers
//         $total_nil_refill = $counts['total']['total'];
//         $stats = [];
//         foreach ($counts as $period => $status_data) {
//             $stats[$period] = [];
//             foreach ($status_data as $status => $scheme_data) {
//                 $stats[$period][$status] = [
//                     'pmuy' => [
//                         'qty' => $scheme_data['pmuy'],
//                         'percent' => $total_nil_refill > 0 ? round(($scheme_data['pmuy'] / $total_nil_refill) * 100, 2) : 0
//                     ],
//                     'non_pmuy' => [
//                         'qty' => $scheme_data['non_pmuy'],
//                         'percent' => $total_nil_refill > 0 ? round(($scheme_data['non_pmuy'] / $total_nil_refill) * 100, 2) : 0
//                     ],
//                     'total' => [
//                         'qty' => $scheme_data['total'],
//                         'percent' => $total_nil_refill > 0 ? round(($scheme_data['total'] / $total_nil_refill) * 100, 2) : 0
//                     ]
//                 ];
//             }
//         }
...
//     }
    ...
    //     $total_nil_refill = $this->get_nillrefill_data(); // Corrected method call
    ...
    //             'total' => ['pmuy' => 0, 'non_pmuy' => 0, 'total' => 0]
    //         ],
    //         'total_consumers' => count($total_nil_refill) // total count
    //     ];
    ...
    //             $status = strtoupper($row['consumer_sub_status']);
    //             $scheme_key = $is_pmuy ? 'pmuy' : 'non_pmuy';

    //             if ($days > 90) {
    //                 if (isset($counts['greater_than_3_months'][$status])) {
    //                     $counts['greater_than_3_months'][$status][$scheme_key]++;
    //                     $counts['greater_than_3_months'][$status]['total']++;
    //                     $counts['greater_than_3_months']['total'][$scheme_key]++;
    //                     $counts['greater_than_3_months']['total']['total']++;
    //                 }
    //             }
    //             if ($days > 180) {
    //                 if (isset($counts['greater_than_6_months'][$status])) {
    //                     $counts['greater_than_6_months'][$status][$scheme_key]++;
    //                     $counts['greater_than_6_months'][$status]['total']++;
    //                     $counts['greater_than_6_months']['total'][$scheme_key]++;
    //                     $counts['greater_than_6_months']['total']['total']++;
    //                 }
    //             }
    //             if ($days > 365) {
    //                 if (isset($counts['greater_than_1_year'][$status])) {
    //                     $counts['greater_than_1_year'][$status][$scheme_key]++;
    //                     $counts['greater_than_1_year'][$status]['total']++;
    //                     $counts['greater_than_1_year']['total'][$scheme_key]++;
    //                     $counts['greater_than_1_year']['total']['total']++;
    //                 }
    //             }
    //         }
    ...
    //     $stats = ['total_consumers' => $counts['total_consumers']];
    ...
    //                 'pmuy' => [
    //                     'qty' => $scheme_data['pmuy'],
    //                     'percent' => $counts['total_consumers'] > 0 ? ($scheme_data['pmuy'] / $counts['total_consumers']) * 100 : 0
    //                 ],
    //                 'non_pmuy' => [
    //                     'qty' => $scheme_data['non_pmuy'],
    //                     'percent' => $counts['total_consumers'] > 0 ? ($scheme_data['non_pmuy'] / $counts['total_consumers']) * 100 : 0
    //                 ],
    //                 'total' => [
    //                     'qty' => $scheme_data['total'],
    //                     'percent' => $counts['total_consumers'] > 0 ? ($scheme_data['total'] / $counts['total_consumers']) * 100 : 0
    //                 ]
    //             ];
    //         }
    //     }
    ...
    //     $stats['all_periods'] = [
    //         'pmuy' => [
    //             'qty' => $counts['greater_than_3_months']['total']['pmuy'] + $counts['greater_than_6_months']['total']['pmuy'] + $counts['greater_than_1_year']['total']['pmuy'],
    //             'percent' => $counts['total_consumers'] > 0
    //                 ? (($counts['greater_than_3_months']['total']['pmuy'] + $counts['greater_than_6_months']['total']['pmuy'] + $counts['greater_than_1_year']['total']['pmuy']) / $counts['total_consumers']) * 100
    //                 : 0
    //         ],
    //         'non_pmuy' => [
    //             'qty' => $counts['greater_than_3_months']['total']['non_pmuy'] + $counts['greater_than_6_months']['total']['non_pmuy'] + $counts['greater_than_1_year']['total']['non_pmuy'],
    //             'percent' => $counts['total_consumers'] > 0
    //                 ? (($counts['greater_than_3_months']['total']['non_pmuy'] + $counts['greater_than_6_months']['total']['non_pmuy'] + $counts['greater_than_1_year']['total']['non_pmuy']) / $counts['total_consumers']) * 100
    //                 : 0
    //         ],
    //         'total' => [
    //             'qty' => $counts['greater_than_3_months']['total']['total'] + $counts['greater_than_6_months']['total']['total'] + $counts['greater_than_1_year']['total']['total'],
    //             'percent' => $counts['total_consumers'] > 0
    //                 ? (($counts['greater_than_3_months']['total']['total'] + $counts['greater_than_6_months']['total']['total'] + $counts['greater_than_1_year']['total']['total']) / $counts['total_consumers']) * 100
    //                 : 0
    //         ]
    //     ];
